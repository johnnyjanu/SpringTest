...
public class Test02Controller {
	...
	@RequestMapping("/2")
	public List<Review> printJson2() {
		
		List<Review> reviewList = new ArrayList<>();
		Review review = new Review("안녕하세요, 가입인사 드립니다.", "hagulu", "안녕하세요. 가입했어요, 앞으로 잘 부탁 드립니다. 활동 열심히 하겠습니다.");
		reviewList.add(review);
		review = new Review("헐 대박", "bada", "오늘 목요일이 었어... 금요일인줄");
		reviewList.add(review);
		review = new Review("오늘 데이트 한 이야기 해드릴게요", "dulumary", "....");
		reviewList.add(review);
		return reviewList;
	...
	//3. ResponseEntity
	@RequestMapping("/3")
	public ResponseEntity<Review> entityResponse() {
		
		Review review = new Review("안녕하세요, 가입인사 드립니다.", "hagulu", "안녕하세요. 가입했어요, 앞으로 잘 부탁 드립니다. 활동 열심히 하겠습니다.");
		
		ResponseEntity<Review> entity = new ResponseEntity<>(review, HttpStatus.INTERNAL_SERVER_ERROR);
		return entity;
